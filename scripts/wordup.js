...
function checkIfWordIsReal(word) {
  ...
  $.ajax({
    ...
    url:
      "http://api.pearson.com/v2/dictionaries/laad3/entries?headword=" + word,
    success: function(response) {
      ...
      let indexOfWord = model.wordSubmissions.findIndex(function(
        elem,
        index,
        array
      ) {
...
function isDisallowedLetter(letter) {
  ...
  let inList = model.allowedLetters.indexOf(letter);
  if (inList === -1) {
    return true;
  } else if (inList !== -1) {
    return false;
